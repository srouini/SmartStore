...
class Model(models.Model):
    ...
    # Network
    network_technology = models.CharField(max_length=200, blank=True, null=True)
    
    # Launch
    launch_announced = models.CharField(max_length=100, blank=True, null=True)
    launch_status = models.CharField(max_length=100, blank=True, null=True)
    
    # Body
    body_dimensions = models.CharField(max_length=100, blank=True, null=True)
    ...
    # Platform
    platform_os = models.CharField(max_length=100, blank=True, null=True)
    platform_chipset = models.CharField(max_length=200, blank=True, null=True)
    platform_cpu = models.CharField(max_length=200, blank=True, null=True)
    platform_gpu = models.CharField(max_length=200, blank=True, null=True)
    
    # Memory
    memory_card_slot = models.CharField(max_length=100, blank=True, null=True)
    memory_internal = models.CharField(max_length=100, blank=True, null=True)
    
    # Camera
    main_camera = models.CharField(max_length=500, blank=True, null=True)
    ...
    # Communications
    comms_nfc = models.CharField(max_length=50, blank=True, null=True)
    comms_infrared_port = models.CharField(max_length=50, blank=True, null=True)
    comms_usb = models.CharField(max_length=100, blank=True, null=True)
    
    # Features
    features_sensors = models.CharField(max_length=500, blank=True, null=True)
    
    # Battery
    battery_type = models.CharField(max_length=100, blank=True, null=True)
    battery_charging = models.CharField(max_length=200, blank=True, null=True)
    
    # Misc
    misc_colors = models.CharField(max_length=200, blank=True, null=True)
...
class PhoneImage(models.Model):
    """Model for storing multiple images for a phone"""
    ...
    def __str__(self):
        return f"Image for {self.phone.name} ({self.id})"
...
class Phone(Product):
    ...
    # Additional physical specifications
    dimensions = models.CharField(max_length=100, blank=True, null=True)  # e.g. "146.7 x 71.5 x 7.4 mm"
    weight_g = models.PositiveIntegerField(blank=True, null=True)  # Weight in grams
    build_materials = models.CharField(max_length=200, blank=True, null=True)  # e.g. "Glass front, aluminum frame"
    sim_type = models.CharField(max_length=100, blank=True, null=True)  # e.g. "Nano-SIM, eSIM"
    ip_rating = models.CharField(max_length=20, blank=True, null=True)  # e.g. "IP68"
    
    def save(self, *args, **kwargs):
        # Auto-generate name based on model if it's a new object
        if not self.pk:  # Only when creating, not updating
            ...
            additional_name = self.name or ""
            
            # If name is empty or matches exactly the model name, generate a new one
            if not additional_name or additional_name == model_name:
                # Use storage and color if available to create a more specific name
                specs = []
                if self.storage_gb:
                    specs.append(f"{self.storage_gb}GB")
                if hasattr(self, 'color') and self.color:
                    specs.append(self.color)
                    
                specs_str = " ".join(specs)
                if specs_str:
                    self.name = f"{brand_name} {model_name} {specs_str}".strip()
                else:
                    self.name = f"{brand_name} {model_name}".strip()
            elif brand_name and model_name and brand_name not in additional_name:
                # If name doesn't already include brand and model, prepend them
                self.name = f"{brand_name} {model_name} {additional_name}".strip()
    ...
    resolution = models.CharField(max_length=100, blank=True, null=True)  # e.g. "1080 x 2400 pixels"
    pixel_density = models.PositiveIntegerField(blank=True, null=True)  # PPI
    refresh_rate = models.PositiveIntegerField(blank=True, null=True)  # Hz
    max_brightness = models.PositiveIntegerField(blank=True, null=True)  # nits
    hdr_support = models.BooleanField(default=False)
    
    # Additional camera specifications
    main_camera_features = models.TextField(blank=True, null=True)  # e.g. "OIS, PDAF, Night mode"
    selfie_camera_features = models.TextField(blank=True, null=True)
    video_recording = models.CharField(max_length=100, blank=True, null=True)  # e.g. "4K@60fps, 1080p@240fps"
    
    # Connectivity
    wifi = models.CharField(max_length=100, blank=True, null=True)  # e.g. "Wi-Fi 802.11 a/b/g/n/ac/6"
    bluetooth = models.CharField(max_length=50, blank=True, null=True)  # e.g. "5.2, A2DP, LE"
    gps = models.CharField(max_length=100, blank=True, null=True)  # e.g. "GPS, GLONASS, GALILEO, BDS"
    nfc = models.BooleanField(default=False)
    radio = models.BooleanField(default=False)
    usb_type = models.CharField(max_length=50, blank=True, null=True)  # e.g. "USB Type-C 3.1"
    
    # Battery and charging
    battery_type = models.CharField(max_length=50, blank=True, null=True)  # e.g. "Li-Po"
    fast_charging = models.CharField(max_length=50, blank=True, null=True)  # e.g. "27W wired"
    wireless_charging = models.CharField(max_length=50, blank=True, null=True)  # e.g. "15W"
    
    # Additional features
    fingerprint = models.CharField(max_length=100, blank=True, null=True)  # e.g. "under display, optical"
    additional_sensors = models.CharField(max_length=200, blank=True, null=True)  # e.g. "accelerometer, gyro, proximity"
    
    # External data
    gsmarena_url = models.URLField(max_length=255, blank=True, null=True)
    other_urls = models.JSONField(default=dict, blank=True, null=True)  # Store additional reference URLs
    ...
    def get_primary_image(self):
        """Returns the primary image for this phone"""
        ...
        # Fallback to the product photo if no images exist
        return self.photo
...
class CaisseOperation(models.Model):
    caisse = models.ForeignKey("api.Caisse", on_delete=models.CASCADE, related_name='operations')
